...
turtles-own [
  ...
  policy-cost     ;; consequence when activating the policy (TUTC and/or education)
  tax-revenue     ;; tax for government from getting sugar
  net-profit
  poor
]
...
to go
  ...
  ask turtles [
    ...
    turtle-eat
    turtle-tutc
    set net-profit (tax-revenue - policy-cost)
    if sugar < 0 [ die ]
    run visualization
    turtle-study
  ]
    ...
    (ifelse
      Max-Education = 1 [
        ...
        vision >= 1 and vision < 2 [
          set policy-cost policy-cost + 0.03
          set vision vision + 0.25
          foreach (range 1 (vision + 1)) [ n -> set vision-points sentence vision-points (list (list 0 n) (list n 0) (list 0 (- n)) (list (- n) 0))]
      ]]
      ...
      Max-Education = 2 [
        (ifelse
          vision >= 1 and vision < 2 [
            ...
            set vision vision + 0.25
            foreach (range 1 (vision + 1)) [ n -> set vision-points sentence vision-points (list (list 0 n) (list n 0) (list 0 (- n)) (list (- n) 0))]]
          vision >= 2 and vision < 4 [
            set policy-cost policy-cost + 0.12
            set vision vision + 0.5
            foreach (range 1 (vision + 1)) [ n -> set vision-points sentence vision-points (list (list 0 n) (list n 0) (list 0 (- n)) (list (- n) 0))]]
        )]
      ...
      Max-Education = 3 [
        (ifelse
          ...
          vision >= 2 and vision < 4 [
            ...
            set vision vision + 0.5
            foreach (range 1 (vision + 1)) [ n -> set vision-points sentence vision-points (list (list 0 n) (list n 0) (list 0 (- n)) (list (- n) 0))]]
          vision >= 4 and vision < 6 [
            set policy-cost policy-cost + 1
            set vision vision + 2
            foreach (range 1 (vision + 1)) [ n -> set vision-points sentence vision-points (list (list 0 n) (list n 0) (list 0 (- n)) (list (- n) 0))]]
        )]
...
to turtle-eat ;; turtle procedure
  ...
  psugar = 0 [set tax 0]
  psugar = 1 [set tax 0]
  psugar = 2 [set tax 0.4]
  psugar = 3 [set tax 1.2]
  psugar = 4 [set tax 1.8])

  set sugar (sugar + psugar - metabolism)
  set tax-revenue (tax-revenue)
  set psugar 0
end
...
to turtle-tutc
  ...
  [
    ...
    [
      set poor true
      set policy-cost (policy-cost + metabolism)
...
"pen-1" 1.0 0 -7500403 true "" "plotxy ticks count turtles with [poor = true]"
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plotxy ticks sum [policy-cost] of turtles"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy ticks sum [net-profit] of turtles"
...
<experiments>
  <experiment name="experiment" repetitions="100" runMetricsEveryStep="false">
    ...
    <metric>count turtles with [poor = true]</metric>
    <metric>sum [policy-cost] of turtles</metric>
    <enumeratedValueSet variable="TUTC?">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="Max-Education">
      <value value="1"/>
      <value value="2"/>
      <value value="3"/>
    </enumeratedValueSet>
  ...
  <experiment name="policy_cost" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
